...
@Injectable()
export class SyntaxCheckerFactory {
  create(language: string) {
    if (language == Language.Python3) return new Python3Verificator();
    if (language == Language.Java17) return new JavaVerificator();
    if (language == Language.C12) return new C12Verificator();
